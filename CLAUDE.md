...
macOS 菜单栏语音输入工具。按快捷键（默认 `Fn`，可自定义）录音，把 Realtime 转写结果注入当前光标。默认短按切换录音，可在设置中开启长按录音。
...
- `HotkeyMonitor.swift` — 录音快捷键全局监听（Fn 走 hidutil 重映射屏蔽输入法切换）
- `RecordingHotkey.swift` — 快捷键模型 / 键码名称表 / UserDefaults 持久化
